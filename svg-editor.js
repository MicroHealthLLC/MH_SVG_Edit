	...
	window.svgEditor = (function($) {
		...
		*/
		editor.loadContentAndPrefs = function () {
			...
			//	var name = 'svgedit-' + curConfig.canvasName;  //NOTE: Original Code
				var name = 'svgedit-fileslisting';
				var cached = JSON.parse(editor.storage.getItem(name));
				if (cached.length > 0) {
			//		editor.loadFromString(cached);    //NOTE: Original Code
			        mh_svgedit = cached;
				}
			}
		...
		*/
		...
		editor.init = function () {
			...
			$.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
				...
				//	editor.setIconSize(size || ($(window).height() < min_height ? 's': 'm')); //NOTE: Original code
					editor.setIconSize(size);
	        ...
	        function editfile(btnid) {
		        ...
		        if (Number(btnid) !== presentfile_id) {
	                ...
	                while ((found === false) && (i < mh_svgedit.length)) {
	                    i += 1;
	                    if (mh_svgedit[i].id === Number(btnid)) {
	                        found = true;
	                    }
	                }
	                if (found === true) {
			...
			searchbtn.addEventListener("click", function () {
			    var searchinput = document.getElementById("searchfiles").value.trim();
			    if (searchinput !== "") {
...
//				return;
				...
				$.pref('iconsize', size);
			//	$('#iconsize').val(size);  //NOTE: Original code
...
//					$('.contextMenu').hide();
			...
			*/
			...
			var clickExport = function() {
				$.select('Select an image type for export: ("PNG" - all browsers; "JPEG" - most browsers; "WEBP" - Chrome and Opera; "BMP" - Firefox; "PDF" - Firefox, Chrome and Opera)', [
			...
			var savePreferences = editor.savePreferences = function() {
				...
				// set icon size
			//	setIconSize($('#iconsize').val());  //NOTE: Original code
				setIconSize('m');
...
'					<defs><linearGradient id="gradbox_"/></defs></svg>', 'text/xml');
		...
		//	}
            ...
            //NOTE: MOST OF THE FOLLOWING KEYBOARD SHORTCUTS ARE CAUSING PROBLEMS WITH SEARCH INPUT!!!
